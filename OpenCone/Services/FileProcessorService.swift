...
class FileProcessorService {
            ...
            for pageIndex in 0..<pageCount {
                ...
                if let attributedString = page.attributedString {
                    do {
                        ...
                        let processedText = try processAttributedStringWithFormat(attributedString, pageNumber: pageIndex + 1)
                        structuredChunks.append(contentsOf: processedText)
                    } catch {
                        logger.log(level: .error, message: "Error processing page \(pageIndex+1): \(error.localizedDescription)")
                    }
                } else {
                    ...
                    if let pageText = page.string {
                        ...
                        logger.log(level: .warning, message: "No text content for page \(pageIndex+1)")
                    }
                }
            }
    ...
    private func processAttributedStringWithFormat(_ attributedString: NSAttributedString, pageNumber: Int) throws -> [(text: String, page: Int, isHeading: Bool, fontSize: CGFloat, rect: CGRect)] {
        var structuredChunks: [(text: String, page: Int, isHeading: Bool, fontSize: CGFloat, rect: CGRect)] = []
        ...
        guard !structuredChunks.isEmpty else {
            throw NSError(domain: "PDFProcessing", code: 100, userInfo: [NSLocalizedDescriptionKey: "No text could be extracted from the attributed string"])
        }
