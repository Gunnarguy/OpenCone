...
final class EmbeddingService: Sendable {
    ...
    func generateEmbeddings(
        ...
        guard !chunks.isEmpty else {
            await log(level: .warning, message: "No chunks provided to generate embeddings")
            return []
        }
        ...
        guard embeddings.count == chunks.count else {
            await log(level: .error, message: "Embedding count mismatch: \(embeddings.count) embeddings for \(chunks.count) chunks")
        ...
        await log(level: .info, message: "Successfully generated \(embeddingModels.count) embeddings")
