...
struct PreviewData {
    ...
    static let sampleDocumentStats: DocumentProcessingStats = {
        ...
        // Add phase timings
        stats.addPhase(phase: .textExtraction, start: baseTime, end: baseTime.addingTimeInterval(5.2))
        stats.addPhase(phase: .chunking, start: baseTime.addingTimeInterval(5.3), end: baseTime.addingTimeInterval(8.1))
        // Corrected enum case name
        stats.addPhase(phase: .embeddingGeneration, start: baseTime.addingTimeInterval(8.2), end: baseTime.addingTimeInterval(25.5))
        // Corrected enum case name
        stats.addPhase(phase: .vectorUpsert, start: baseTime.addingTimeInterval(25.6), end: baseTime.addingTimeInterval(30.0))
    ...
    static var sampleDocumentsViewModel: DocumentsViewModel {
        ...
        let pinecone = PineconeService(apiKey: "dummy-pinecone-key", projectId: "dummy-project-id") // Provide API key and project ID
    ...
    static var sampleProcessingViewModel: ProcessingViewModel {
        ...
        logger.log(level: .warning, message: "API Key for OpenAI not found. Please add it in Settings.", context: "Configuration")
        logger.log(level: .info, message: "Processing document: Sample Report.pdf", context: "Document Processing")
        logger.log(level: .success, message: "Document 'Sample Report.pdf' processed successfully.", context: "Document Processing")
        logger.log(level: .error, message: "Failed to extract text from 'Corrupted Data.docx'.", context: "FileProcessorService")
    ...
    static let sampleSearchResults: [SearchResultModel] = [
        SearchResultModel(
            content: "The primary goal of the project is to enhance user engagement through personalized recommendations.",
            sourceDocument: "Project Proposal.pdf",
            score: 0.92,
            metadata: ["page": "3", "chunk_index": "5"]
        ),
        SearchResultModel(
            content: "Recommendations are generated based on user interaction history and content similarity.",
            sourceDocument: "Technical Spec.docx",
            score: 0.85,
            metadata: ["section": "4.2", "chunk_index": "12"]
        ),
        SearchResultModel(
            content: "User engagement metrics include click-through rate and time spent on recommended items.",
            sourceDocument: "Project Proposal.pdf",
            score: 0.71,
            metadata: ["page": "8", "chunk_index": "21"]
        ),
        SearchResultModel(
            content: "Future work involves exploring collaborative filtering techniques for improved accuracy.",
    ...
    static var sampleSearchViewModel: SearchViewModel {
        ...
        let pinecone = PineconeService(apiKey: "dummy-pinecone-key", projectId: "dummy-project-id")
        ...
        viewModel.selectedResults = sampleSearchResults.filter { $0.id == sampleSearchResults[1].id }
        viewModel.generatedAnswer = "The system enhances user engagement by providing personalized recommendations based on interaction history and content similarity. Key metrics include CTR and time spent."
...
#Preview("Document Stats") {
    ...
    VStack(alignment: .leading) {
        ...
        Text("Total Duration: \(formattedDuration(PreviewData.sampleDocumentStats.totalProcessingTime))")
    }
    .padding()
    .withTheme()
}
